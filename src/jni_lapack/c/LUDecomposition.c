...
/* LU */
JNIEXPORT jint JNICALL Java_JAMAJni_jni_1lapack_LUDecomposition_dgetrf
...
JNIEXPORT void JNICALL Java_JAMAJni_jni_1lapack_LUDecomposition_dgetrs
...
JNIEXPORT jint JNICALL Java_JAMAJni_jni_1lapack_LUDecomposition_dgetri
  (JNIEnv *env, jclass obj, jint layout, jint n, jdoubleArray ja, jint lda,
   jintArray jipiv, jdoubleArray jwork, jint lwork, jintArray jinfo)
{
    double *a = (*env)-> GetDoubleArrayElements (env, ja, NULL);
    int *ipiv = (*env)-> GetIntArrayElements(env,jipiv,NULL);
    ...
    if (layout == jniColMajor){
        result = dgetri_(&n, a, &lda, ipiv, work, &lwork, info);
    } else if (layout == jniRowMajor) {
        if (lda < n) {
            fprintf(stderr, "** Illegal value of lda for row-major layout\n");
            return -1;
        ...
        int lda_t = n;
        RCswitch(a, a_t, n, n, lda, lda_t);
        result = dgetri_(&n, a_t, &lda_t, ipiv, work, &lwork, info);
...
/*
JNIEXPORT void JNICALL Java_JAMAJni_jni_1lapack_jniLAPACK_dpotrf
...
JNIEXPORT jint JNICALL Java_JAMAJni_jni_1lapack_jniLAPACK_dpotri
  (JNIEnv *env, jclass obj, jint layout, jchar juplo, jint n, jdoubleArray ja,
   jint lda, jintArray jinfo)
{
    double *a = (*env)-> GetDoubleArrayElements (env, ja, NULL);
...
JNIEXPORT jint JNICALL Java_JAMAJni_jni_1lapack_jniLAPACK_dpotrs
